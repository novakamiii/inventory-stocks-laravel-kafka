...
<head>
    ...
    <title>{{ config('app.name', 'Inventory Stock Management') }}</title>
...
<body style="background-color: white;">
    ...
    <div>
        <p><strong>Kafka Status:</strong>
            <span id="kafka-status">{{ $kafkaStatus ? 'Connected' : 'Disconnected (Fallback Mode)' }}</span>
        </p>
    </div>
    ...
    <table border="1" id="products-table">
        ...
        <tbody>
            ...
            <tr data-product-id="{{ $product->id }}">
                ...
                <td class="updated-at">{{ $product->updated_at->format('Y-m-d H:i:s') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <script>
        ...
        async function simulateOrder() {
            ...
            try {
                const response = await fetch('{{ route('inventory.simulate') }}', {
                    method: 'POST',
                    headers: {
        ...
        async function simulateLowStock() {
            ...
            try {
                const response = await fetch('{{ route('inventory.simulate-low-stock') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': getCsrfToken()
        ...
        async function checkLowStock() {
            ...
            try {
                const response = await fetch('{{ route('inventory.check-low-stock') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': getCsrfToken()
        ...
        async function restockAll() {
            ...
            try {
                const response = await fetch('{{ route('inventory.restock') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': getCsrfToken()
